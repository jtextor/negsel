...
long dfs( const map<int, vector<int> >& edges, int start, const set<int> & end, bool clean=false ){
	static map< int, long > n;
	if( clean ){
		n.clear();
	}
...
int main( int argc, char * argv[] )
{
	...
	long npaths = 0;
	npaths += dfs( edges, initial, accepting, true );
	cout << npaths << endl;
}
